...
import { FiMenu, FiZap, FiHome } from 'react-icons/fi';
        ...
        <div className="flex flex-wrap md:flex-nowrap justify-between items-center h-auto min-h-[56px] sm:min-h-[64px] py-2 gap-2 md:gap-0">
          ...
          <nav className="hidden md:flex items-center space-x-6 lg:space-x-8">
            <Link
              href="/"
              className={cn(
                "relative text-base lg:text-lg font-medium transition-all duration-300 group flex items-center gap-2",
                pathname === '/' ? "text-primary-400" : "text-gray-400 hover:text-primary-400"
              )}
            >
      ...
      >
        <div className="px-3 sm:px-4 py-2 space-y-1 bg-gray-900/95 backdrop-blur-xl border-t border-gray-800">
          <Link
            href="/"
            className={cn(
              "flex items-center gap-2 px-3 py-2 rounded-lg text-sm sm:text-base font-medium transition-colors duration-200",
              pathname === '/'
